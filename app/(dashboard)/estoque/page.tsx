...
export default async function EstoquePage() {
  ...
  const [{ data: produtos }, { data: movimentos }] = user
    ? await Promise.all([
        supabase.from('produtos_fiscais').select('*').eq('user_id', user.id).eq('ativo', true).order('descricao'),
        supabase.from('movimentos_estoque').select('*').eq('user_id', user.id).order('data', { ascending: false }).limit(50),
      ])
    : [{ data: [] }, { data: [] }]

  const prods = (produtos ?? []) as {
    id: string; codigo: string; descricao: string; ncm: string; unidade: string
    preco_unitario: number; estoque: number; estoque_minimo: number
  }[]

  const movs = (movimentos ?? []) as {
    id: string; data: string; produto_nome: string; produto_codigo: string
    tipo: 'entrada' | 'saida'; quantidade: number; motivo: string; nf_referencia?: string
  }[]

  const abaixoMinimo = prods.filter((p) => p.estoque <= p.estoque_minimo)
  const valorTotal = prods.reduce((s, p) => s + p.estoque * p.preco_unitario, 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total de Produtos',  value: String(prods.length),                   sub: 'ativos no cadastro' },
          { label: 'Total em Estoque',   value: String(prods.reduce((s, p) => s + p.estoque, 0)), sub: 'unidades' },
          ...
          <Card key={label} className={cn(alert && 'border-yellow-300 dark:border-yellow-700')}>
            <CardContent className="pt-6">
              ...
              {alert && value !== '0'
                ? <p className="text-xs text-yellow-600 dark:text-yellow-500 mt-1 flex items-center gap-1"><AlertTriangle className="h-3 w-3" />Atenção necessária</p>
      ...
      {/* Posição de estoque */}
      <Card>
        <CardHeader><CardTitle className="text-base">Posição de Estoque</CardTitle></CardHeader>
        <CardContent className="p-0">
          {prods.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <Link href="/nfe-entradas/nova" className="mt-4 text-blue-600 font-medium text-sm hover:underline">
                  ...
                  <tr className="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                    {['Código','Produto','Un.','Estoque Atual','Mínimo','Valor Unit.','Total','Situação'].map((h) => (
                      <th key={h} className={cn('px-6 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide',
                        ['Estoque Atual','Mínimo','Valor Unit.','Total'].includes(h) ? 'text-right' : 'text-left')}>{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {prods.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className={cn('hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors', abaixo && 'bg-yellow-50/50 dark:bg-yellow-900/10')}>
                        ...
                        <td className="px-6 py-4">
                          {abaixo
                            ? <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400"><AlertTriangle className="h-3 w-3" />Baixo</span>
                            : <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">Normal</span>}
                        </td>
                      </tr>
                    )
                  })}
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {movs.map((m) => (
                    <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      <td className="px-6 py-3 text-gray-600 dark:text-gray-400">{formatTs(m.data)}</td>
