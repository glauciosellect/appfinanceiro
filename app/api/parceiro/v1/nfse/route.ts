...
// user_id, e chama a MESMA função emitirNFSe() do Focus NFe — nenhuma
...
export async function POST(req: NextRequest) {
  ...
  const ref = `nfse_parc${empresa.id.replace(/-/g, '').slice(0, 8)}_${numero_rps}`
  ...
  const payload = {
    ...
    regime_tributario_prestador: empresa.regime_tributario ?? '1',
    codigo_municipio_prestador: empresa.codigo_municipio ?? undefined,
  }
  ...
  try {
    ...
    console.log('[parceiro/nfse] retorno Focus NFe:', JSON.stringify(retorno))
  } catch (err) {
  ...
  if (retorno.erros && retorno.erros.length > 0) {
    ...
    console.error('[parceiro/nfse] erros Focus NFe:', erroMsg)
